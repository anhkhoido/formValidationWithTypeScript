import { Province } from "./model/geography/province";
...
function populateProvince() : void {
    var myProvinces : Province[] =[
            new Province("Ontario", "ON"),
            new Province("Quebec", "QC")
    ];
    myProvinces.forEach(province => {
        var option = document.createElement('option');
        option.innerText = province.name;
        option.value = province.abbreviation;
        provincesSelectTag.appendChild(option);
    });
}
